...
class TrainManager():
    ...
    def __init__(self,env,episodes=1001,lr=0.001,gamma=0.9,e_greed=0.1):
        self.env = env
        n_act = env.action_space.n        #2
        n_obs = env.observation_space.shape[0]   #    观测空间为4维
        q_func = modules.MLP(n_obs,n_act)
        optimizer = torch.optim.AdamW(q_func.parameters(),lr=lr)     #神经网络的优化器   adamw优化器可以收敛速度更快的同时波动幅度更小
    ...
    def train_episode(self):
        ...
        obs = torch.FloatTensor(obs)        #转换为tensor
        while True:
